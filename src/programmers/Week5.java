...
public class Week5 {
	...
	[1, 2, 3, 9, 10, 12]	7		2
     */
    public int day33(int[] scoville, int K) {
        int answer = 0;
        PriorityQueue<Integer> q=new PriorityQueue<Integer>(); //우선순위 큐 생성
        for(int i=0;i<scoville.length;i++){
            q.add(scoville[i]);
        }
        while(q.peek()<K){ //가장 작은 스코빌지수가 K보다 작은동안 계속
            if(q.size()==1) return -1; //하지만 큐의 사이즈가 1일경우는 더이상 합칠수 없으므로 -1 리턴
            int a=q.poll();
            int b=q.poll();
            int newScoville=a+b*2;
            if(newScoville==0){ //두 수가 모두 0일때도 아무리 합쳐도 K이상 불가능
                return -1;
            }
            q.add(newScoville); //큐에 새로 추가(우선순위로 인해 자리가 옮겨짐)
            answer++;
        }
        return answer;
    }
